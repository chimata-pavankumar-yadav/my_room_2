...
const validationSchema = yup.object().shape({
  ...
  rent: yup.number().required('rent is required').positive(' groceries amount must be positive number'),
  bills:yup.number().required('bills amount required'),
  groceries:yup.number().required('groceries amount required')
 ...
 const submitData = (data) => {
  ...
  db.transaction(
    tx => {
      if (data) {
        tx.executeSql(
          "insert into room_amount (date, Name, rent, groceries, bills, total) values (?,?,?,?,?,?)",
          [insertDate, data.name, data.rent, data.groceries, data.bills, total],
          (_, insertResult) => {
            // Handle successful insertion
            console.log("Data inserted successfully into room_amount");
          },
          (_, insertError) => {
            // Handle insertion error
            console.error("Error inserting data into room_amount:", insertError);
          }
        );
        ...
        tx.executeSql(
          ...
          (_, { rows: { _array } }) => {
            if (_array.length > 0) {
              // If there are rows in the result, update the existing row
              tx.executeSql(
                "UPDATE groceries_amount SET totalAmount = totalAmount + ?, spentAmount= spentAmount + ?  WHERE id = 1",
                [data.groceries,0],
                (_, updateResult) => {
                  // Handle successful update
                  console.log("Groceries amount updated successfully");
              ...
              // If no rows are returned, insert a new row
              tx.executeSql(
                'INSERT INTO groceries_amount (totalAmount, spentAmount) VALUES (?,?)',
                [data.groceries,0],
                (_, insertResult) => {
                  // Handle successful insertion
                  console.log("Data inserted successfully into groceries_amount");
                  selectDataFromDatabase(tx);
                },
                (_, insertError) => {
                  // Handle insertion error
                  console.error("Error inserting data into groceries_amount:", insertError);
                }
              );
            }
          },
        ...
        tx.executeSql(
          ...
          (_, selectError) => {
            // Handle select error
            console.error("Error selecting data from room_amount:", selectError);
          }
        );
      }
    },
    (_, transactionError) => {
      ...
      console.error("Error executing database transaction:", transactionError);
    }
  );
